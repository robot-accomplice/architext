...
function main() {
  ...
  if (model.roadmap && model.releases) {
    validateRoadmapReleaseTargets(model.roadmap, model.releases, errors);
  }
...
function validateRoadmapReleaseTargets(roadmap, releases, errors) {
  const releaseIds = new Set(releases.index.releases.map((release) => release.id));
  for (const item of roadmap.items) {
    if (item.targetReleaseId) requireKnown(item.targetReleaseId, releaseIds, `roadmap item ${item.id}.targetReleaseId`, errors);
  }
}
