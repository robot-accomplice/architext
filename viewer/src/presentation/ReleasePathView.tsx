...
export function ReleasePath({
  ...
  const collapsedMilestoneCount = milestoneIds.filter((id) => collapsedMilestoneIds.has(id)).length;
  const allMilestonesCollapsed = milestoneIds.length > 0 && collapsedMilestoneCount === milestoneIds.length;
  const allMilestonesExpanded = collapsedMilestoneCount === 0;

  return (
    <div className="release-path">
      <div className="release-path-controls" aria-label="Release Path collapse controls">
        <span>{collapsedMilestoneCount}/{milestoneIds.length} collapsed</span>
        <button
          type="button"
          disabled={allMilestonesCollapsed}
          onClick={() => setCollapsedMilestoneIds(collapseAllReleasePathMilestones(milestoneIds))}
        >
          Collapse all
        </button>
        <button
          type="button"
          disabled={allMilestonesExpanded}
          onClick={() => setCollapsedMilestoneIds(expandAllReleasePathMilestones())}
        >
          Expand all
        </button>
      </div>
