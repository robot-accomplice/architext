...
test("schema migration planning is empty when data is current", () => {
  const plan = schemaMigrationPlan({ currentVersion: "1.4.0", targetVersion: "1.4.0" });
...
test("schema migration planning classifies additive migrations", () => {
  const plan = schemaMigrationPlan({ currentVersion: "1.3.0", targetVersion: "1.4.0" });
  ...
  assert.match(plan.pending[0].summary, /apply additive schema migration 1\.3\.0 -> 1\.4\.0/);
});

test("schema migration planning classifies breaking migrations", () => {
  const plan = schemaMigrationPlan({ currentVersion: "1.4.0", targetVersion: "2.0.0" });

  assert.equal(plan.pending[0].kind, "breaking");
  assert.match(plan.pending[0].summary, /apply breaking schema migration 1\.4\.0 -> 2\.0\.0/);
});

test("schema migration planning refuses newer target data", () => {
  const plan = schemaMigrationPlan({ currentVersion: "2.0.0", targetVersion: "1.4.0" });
