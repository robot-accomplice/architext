...
//!   - GET  /api/tools        (which enrichment tools are installed)
//!   - POST /api/tools/run    (mutation: run magma / the slop-ferret pipeline)
...
pub fn build_router(state: AppState, farm: Farm, hub: Option<WatchHub>) -> Router {
    ...
    let router = Router::new()
        ...
        .route("/api/tools/run", post(handlers::tools::post_tools_run))
