...
fn doctor_repairs_for_status(status: &Value) -> Value {
    ...
    // instructionRules — attribute rewrite changes to the file they actually
    // touch (the change string embeds it, generated by
    // planned_instruction_rule_migration); rule migrations write rules.json.
    for change in status["instructionRules"]["repairChanges"].as_array().into_iter().flatten() {
        if let Some(s) = change.as_str() {
            let file = s
                .strip_prefix("rewrite ")
                .and_then(|rest| rest.strip_suffix(" to point at docs/architext/data/rules.json"))
                .unwrap_or("docs/architext/data/rules.json");
            repairs.push(json!({
